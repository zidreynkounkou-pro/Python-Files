try:
    ...
    def wind_chill(t,v):
        calc_wind_chill = 35.74 + (0.6215) * t - 35.75 * (v ** 0.16) + 0.4275 * t * (v ** 0.16) 
        return calc_wind_chill
    ...
    if c_or_f.lower() == "f":
        for v in range(5,61,5):
            calc_wind_chill = wind_chill(t,v)
            print(f"At temperature {t: .1f}F, and wind speed {v} mph, the windchill is: {calc_wind_chill: .2f}F")
    else:
        for v in range(5,61,5):
            calc_wind_chill = celsius_into_fahrenheit(t,v)
            print(f"At temperature {f: .1f}F, and wind speed {v} mph, the windchill is: {calc_wind_chill: .2f}F")
except:
