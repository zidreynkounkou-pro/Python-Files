...
def test_read_dictionary():
    ...
    file_path = mktemp(dir=".", prefix="not", suffix=".csv")
    with pytest.raises(FileNotFoundError):
        call_read_dictionary(file_path, I_NUMBER_INDEX)
    ...
    file_path = path.join(path.dirname(__file__), "students.csv")
    students_dict = call_read_dictionary(file_path, I_NUMBER_INDEX)
...
def call_read_dictionary(file_path, key_column_index):
    ...
    if length == min_len:
        ...
        dictionary = read_dictionary(file_path, key_column_index)
    return dictionary
